...
const FacultySignup = () => {
  ...
  const handleSignup = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:4000/faculty/signup', { name, email, password });
  ...
  return (
    <>
      ...
      <div className="container login-container">
        <h2 className="text-center">Faculty Signup</h2>
        <form onSubmit={handleSignup} className="form-signin">
          ...
          <p className="text-center mt-3">
            Already have an account? <Link to="/login/faculty" className="toggle-button">Login</Link>
          </p>
        </form>
      </div>
      <Footer />
